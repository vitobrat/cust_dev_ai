...
from tests.unit.infrastructure.schema import DummyOutputSchema, DummyStateSchema
...
def test_base_graph_initializes_dependencies_and_builds_graph(
    ...
    assert concrete_graph._llm_adapter is llm_adapter_instance
    assert concrete_graph._prompt_builder is mock_prompt_builder
    assert concrete_graph._graph is compiled_state_graph
...
async def test_process_invokes_graph_and_validates_output(
    ...
    compiled_state_graph.ainvoke.return_value = {'output': 'done'}
    ...
    compiled_state_graph.ainvoke.assert_awaited_once_with(
        payload,
...
async def test_process_includes_langfuse_handler_in_callbacks(
    ...
    compiled_state_graph.ainvoke.return_value = {'output': 'ok'}
    state = DummyStateSchema(input='value').model_dump()
    await graph.process(state)
    config = compiled_state_graph.ainvoke.call_args.kwargs['config']
...
async def test_process_wraps_graph_invocation_errors(
    ...
    compiled_state_graph.ainvoke.side_effect = ValueError('boom')
...
async def test_process_raises_when_graph_returns_none(
    ...
    compiled_state_graph.ainvoke.return_value = None
    with pytest.raises(GraphError):
        await graph.process(DummyStateSchema(input='value').model_dump())
...
async def test_process_raises_on_output_validation_failure(
    ...
    compiled_state_graph.ainvoke.return_value = {'unexpected': 'value'}
    with pytest.raises(GraphError):
        await graph.process(DummyStateSchema(input='value').model_dump())
