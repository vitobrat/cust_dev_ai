...
async def test_analyse_user_prompt_updates_state(
    ...
    """Ensure analyse node returns a partial dict with analysis_result."""
    mock_llm_adapter = container.infrastructure.llm_adapter()
    mock_persona_prompt_builder = container.domain.persona.prompt_builder()

    node_result = await user_segment_search_graph._analyse_user_prompt(user_segment_search_state)
    ...
    assert node_result["analysis_result"] == "llm adapter ainvoke"
...
async def test_find_user_segment_appends_history_item(
    ...
    """Confirm the find node returns updated segments_history."""
    mock_llm_adapter = container.infrastructure.llm_adapter()
    mock_persona_prompt_builder = container.domain.persona.prompt_builder()
    ...
    user_segment_search_state["segments_history"] = []
    node_result = await user_segment_search_graph._find_user_segment(user_segment_search_state)
    ...
    assert len(node_result["segments_history"]) == 1
    last_segment = node_result["segments_history"][-1]
...
async def test_verify_user_segment_sets_result(
    ...
    """Verify node should return a dict with verification_result."""
    mock_llm_adapter = container.infrastructure.llm_adapter()
    mock_persona_prompt_builder = container.domain.persona.prompt_builder()
    ...
    node_result = await user_segment_search_graph._verify_user_segment(user_segment_search_state)
    ...
    assert node_result["verification_result"] is verification_state
...
async def test_output_node_returns_final_segment(
    ...
    """Output node should produce a dict with segment name and description."""
    user_segment_search_state["segments_history"] = [user_segment_state]
